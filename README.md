...
| Your second machine gets `401` on everything | auth was turned on and it is using an old or missing token | mint a token for that device (§6.4), or rotate `AUTH_JWT_SECRET` if a token may have leaked — the API accepts any key **only** while `AUTH_USE_AUTH=false` |
